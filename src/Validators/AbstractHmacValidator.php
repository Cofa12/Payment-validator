 ...
 * signing string, HMAC it, compare in constant time — so subclasses only
 * declare *what* is signed and *where* the signature lives.
 */
abstract class AbstractHmacValidator implements SignatureValidator
{
    ...
    public function __construct(
        ...
        protected readonly string $algorithm = 'sha512',
    ) {
        $secret = trim($secret);
        ...
        if (! in_array($this->algorithm, hash_hmac_algos(), true)) {
            throw InvalidConfigurationException::unsupportedAlgorithm($this->algorithm);
        }
    ...
    /** The exact string fed to HMAC — the first thing to inspect when a signature will not match. */
